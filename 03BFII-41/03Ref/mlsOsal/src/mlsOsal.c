...
Void mlsOsalDelay100us(UInt32 dw_100_us)
{
#ifdef MLS_OSAL_FREE_RTOS
	vTaskDelay(dw_100_us);
#endif
}
...
{
#ifdef MLS_OSAL_FREE_RTOS
	vTaskDelay(((wHours*3600000)+(wMinutes*60000)+(wSeconds*1000)+wMiliseconds)*10);
#endif
...
UInt32 mlsOsalGetElapseTime(UInt32 dwFrom)
{
	...
	if (curTime >= dwFrom)
	{
		return (curTime - dwFrom) / 10;
	}
	else
	{
		return ((UInt32)0xFFFFFFFF - dwFrom + curTime) / 10;
	}
}
