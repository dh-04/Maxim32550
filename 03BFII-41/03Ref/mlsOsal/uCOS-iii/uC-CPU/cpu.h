...
#include "../uC-BSP/cpu_cfg.h"
#include "../uC-BSP/cpu_bsp.h"
...
#define  CPU_INT_DIS()         do { cpu_sr = CPU_SR_Save(); /*cpu_sr = __get_PRIMASK(); __disable_interrupt();*/ } while (0)
#define  CPU_INT_EN()          do { CPU_SR_Restore(cpu_sr);/*__set_PRIMASK(cpu_sr);*/ } while (0)  /* Restore CPU status word.                      */
