...
/* wc_AesSetKey */
                  ...
                  int dir)
    {
        ...
        aes->key_length = keylen;
        memcpy(aes->keys, userKey, keylen);
        memcpy(aes->iv, iv, keylen);
...
/* AES-CBC */
    ...
    int wc_AesCbcEncrypt(Aes* aes, byte* out, const byte* in, word32 sz)
    {
    	return ucl_aes_cbc(out, in, sz, aes->keys, aes->key_length, aes->iv, UCL_CIPHER_ENCRYPT);
    }
    int wc_AesCbcDecrypt(Aes* aes, byte* out, const byte* in, word32 sz)
    {
    	return ucl_aes_cbc(out, in, sz, aes->keys, aes->key_length, aes->iv, UCL_CIPHER_DECRYPT);
    }
#else
    int wc_AesCbcEncrypt(Aes* aes, byte* out, const byte* in, word32 sz)
    {
