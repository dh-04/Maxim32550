...
DL_ERR _pack_iso_Nibble ( DL_UINT16                    iField,
		  const DL_ISO8583_MSG        *iMsg,
		  const DL_ISO8583_FIELD_DEF  *iFieldDefPtr,
		  DL_UINT8                   **ioPtr );
...
struct DL_ISO8583_TYPE_S
{
	...
	DL_ERR    (*packFuncBin  )(DL_UINT16,const DL_ISO8583_MSG*,const DL_ISO8583_FIELD_DEF*,DL_UINT8**);
};
...
/* ISO_N    */ {_unpack_iso_ASCHEX,_pack_iso_ASCHEX,_pack_iso_ASCHEX},
/* ISO_NS   */ {_unpack_iso_BINARY,_pack_iso_BINARY,_pack_iso_ASCHEX},
/* ISO_XN   */ {_unpack_iso_ASCHEX,_pack_iso_ASCHEX,_pack_iso_ASCHEX},
/* ISO_A    */ {_unpack_iso_ASCII ,_pack_iso_ASCII ,_pack_iso_ASCHEX},
/* ISO_AN   */ {_unpack_iso_ASCII ,_pack_iso_ASCII ,_pack_iso_ASCHEX},
/* ISO_ANS  */ {_unpack_iso_ASCII ,_pack_iso_ASCII ,_pack_iso_ASCHEX},
/* ISO_ANSB */ {_unpack_iso_ASCII ,_pack_iso_ASCII ,_pack_iso_ASCHEX},
/* ISO_ANP  */ {_unpack_iso_ASCII ,_pack_iso_ASCII ,_pack_iso_ASCHEX},
/* ISO_B    */ {_unpack_iso_BINARY,_pack_iso_BINARY,_pack_iso_ASCHEX},
/* ISO_Z    */ {_unpack_iso_BINARY,_pack_iso_Nibble,_pack_iso_Nibble},
/* ISO_BMAP */ {_unpack_iso_BITMAP,_pack_iso_BITMAP,_pack_iso_ASCHEX} };
								...
								DL_UINT8                 **ioPtr )
{
	...
	if(iMsg->field[iField].func == SET_BY_STR && fieldDefPtr->fieldType == kDL_ISO8583_NS)
		err = fieldTypePtr->packFuncBin(iField,iMsg,fieldDefPtr,ioPtr);
	else
						  ...
						  DL_UINT8                   **ioPtr )
{
	...
	if(fieldPtr->func == SET_BY_STR && iFieldDefPtr->fieldType == kDL_ISO8583_NS)
	{
		if(iFieldDefPtr->varLen != kDL_ISO8583_FIXED)
			return kDL_ERR_OTHER; //temporarily actLen = actLen / 2;
		else
			reqLen        = iFieldDefPtr->len * 2;
	}
						    ...
						    DL_UINT8                   **ioPtr )
{
	...
	DL_UINT16  size       = 0;
	DL_UINT8  *tmpDataPtr = NULL;
	DL_UINT16 length = iFieldDefPtr->len / 2;
	length = (length * 2 == iFieldDefPtr->len) ? length : length + 1;

	/* variable length handling */
	err = VarLen_Get(&tmpPtr,iFieldDefPtr->varLen,length,&size);
						  ...
						  DL_UINT8                   **ioPtr )
{
	...
	/* variable length handling */
	err = VarLen_Put(iFieldDefPtr->varLen,actLen * 2,&reqLen,&tmpPtr);
