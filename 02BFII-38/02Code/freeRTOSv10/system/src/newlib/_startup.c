...
// defined in linker script (need for isr_vector section)
extern unsigned int __isr_vector_start;
extern unsigned int __vectors_start;
extern unsigned int __vectors_end;

// defined in linker script (need for ram code section)
extern unsigned int __section_ram_code_load_start;
extern unsigned int __section_ram_code_start;
extern unsigned int __section_ram_code_end;

// defined in linker script (need for data section)
...
void __attribute__ ((section(".after_vectors"),noreturn,weak, naked))
_start (void)
{
	asm volatile(" LDR r0, =_estack");			//Get value for stack pointer
	asm volatile( "MSR msp, r0");			    // Initialize SP
	asm volatile( "bl _start_c");			    //
}
...
_start_c (void)
{
  ...
  // Realloc isr_table to RAM
  if(&__isr_vector_start != &__vectors_start)
	  __initialize_data (&__isr_vector_start, &__vectors_start, &__vectors_end);

  if(&__section_ram_code_load_start != &__section_ram_code_start)
	  __initialize_data (&__section_ram_code_load_start, &__section_ram_code_start, &__section_ram_code_end);
