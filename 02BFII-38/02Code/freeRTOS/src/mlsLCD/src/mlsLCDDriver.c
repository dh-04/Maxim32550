...
mlsErrorCode_t mlsLCDDisplayCustomQRCode(Int8 * szSourceSring, uint8_t * QRCodeBufeer, uint8_t * tempBuffer)
{
	...
	bool ok = qrcodegen_encodeText((const char*)szSourceSring,
	    tempBuffer, QRCodeBufeer, qrcodegen_Ecc_MEDIUM,
	...
	width = qrcodegen_getSize(QRCodeBufeer);
	...
	for(y = 0; y < width; y++)
	{
		for(x = 0; x < width; x++)
		{
			if (qrcodegen_getModule(QRCodeBufeer, x, y))
			{
				for(i = 0;i < scale; i++)
				{
					for(t = 0;t < scale; t++)
