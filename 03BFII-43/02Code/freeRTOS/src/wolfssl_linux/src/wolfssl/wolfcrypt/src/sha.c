...
#ifdef STYL_MAXIM_SHA
	int wc_InitSha(Sha* sha)
	{
		return ucl_sha1_init(sha);
	}
	int wc_ShaUpdate(Sha* sha, const byte* data, word32 len)
	{
		return ucl_sha1_core(sha, (byte *)data, len);
	}

	int wc_ShaFinal(Sha* sha, byte* hash)
	{
		return ucl_sha1_finish(hash, sha);
	}
#else
