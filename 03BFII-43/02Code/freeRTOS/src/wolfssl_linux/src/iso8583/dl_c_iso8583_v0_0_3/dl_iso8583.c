...
#include "../src/mlsRtc/inc/mlsRtc.h"
...
// returns: error code
DL_ERR DL_ISO8583_MSG_SetField_Str (const DL_ISO8583_HANDLER *iHandler,
									DL_UINT16       iField,
						            ...
						            DL_ISO8583_MSG *ioMsg )
{
	...
	err = DL_ISO8583_MSG_SetField_Bin(iField,
									  ...
									  ioMsg);
	ioMsg->field[iField].func = SET_BY_STR;
					                 ...
					                 DL_ISO8583_MSG *ioMsg )
{
	...
	ioMsg->field[iField].func = SET_BY_BINARY;
...
//			fprintf(iOutFile,"%s%s",iMsg->field[i].ptr,tmpEOL);
			fprintf(iOutFile,"[%03d] ",
					(int)i);
			for(j = 0; j < iMsg->field[i].len; j++)
				fprintf(iOutFile, " 0x%x ", iMsg->field[i].ptr[j]);
...
//char DL_ISO8583_MSG_DumpEMB ( FILE                     *iOutFile,
//					       const char               *_iEolStr,
//					       const DL_ISO8583_HANDLER *iHandler,
//					       const DL_ISO8583_MSG     *iMsg)
//{
...
//	char     *tmpEOL = _iEolStr == NULL ? "\n" : _iEolStr;
//	char errorCode = 20;
//	unsigned char hhmmss[3];
//	unsigned char MMDD[2];
//	mlsDateTime_t dateTime;
//	unsigned long unixTime;
//
//	memcpy(gHostResponse.response_code, "FF", 2);
//
//	/* for each field */
//	for ( i=0 ; i<(iHandler->fieldItems) ; i++ )
//	{
...
//				if(i == 12)
//				{
//					memcpy(hhmmss, iMsg->field[i].ptr, MLS_MACRO_MIN(iMsg->field[i].len, sizeof(hhmmss)));
//					dateTime.hour = (hhmmss[0] >> 4) * 10 + (hhmmss[0] & 0x0F);
//					dateTime.minute = (hhmmss[1] >> 4) * 10 + (hhmmss[1] & 0x0F);
//					dateTime.second = (hhmmss[2] >> 4) * 10 + (hhmmss[2] & 0x0F);
//				}
//				else if (i == 13)
//				{
//					memcpy(MMDD, iMsg->field[i].ptr, MLS_MACRO_MIN(iMsg->field[i].len, sizeof(MMDD)));
////					dateTime.year = 2018;
////					dateTime.month = (MMDD[0] >> 4) * 10 + (MMDD[0] & 0x0F);
////					dateTime.dayOfMonth = (MMDD[1] >> 4) * 10 + (MMDD[1] & 0x0F);
////					mlsGregorianTimeToUnixTime(&unixTime, dateTime);
////					mlsRtcSet(unixTime, PRIOR_LOW);
//				}
//			}
//			if(i == 0)
//			{
//				memcpy(gHostResponse.function_code, iMsg->field[i].ptr, 2);
//			}
//			if(i == 39)
//			{
//				//get field 39
//				memcpy(gHostResponse.response_code, iMsg->field[i].ptr, 2);
//				if((iMsg->field[i].ptr[0] == 0x30) && (iMsg->field[i].ptr[1] == 0x30))
//				{
//					errorCode = 0;
//				}
//				else
...
//			if(i == 3)
//			{
//				memcpy(gHostResponse.processing_code, iMsg->field[i].ptr, 3);
//			}
//
//			if(i == 4)
//			{
//				memcpy(gHostResponse.balance, iMsg->field[i].ptr, 6);
//			}
//
//			if(i == 57)
//			{
...
//			}
////			lite_printf(1, "\r\n%03x: ", i);
////			print_buffer(1, iMsg->field[i].ptr, iMsg->field[i].len);
//		}
//
//	} /* end-for(i) */
//	return errorCode;
